...
export function BottomNav() {
  ...
  const newTripActive = pathname === '/trips/new';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <div
          className="flex items-center gap-1 rounded-full border border-white/[0.07] bg-ink-200/90 px-2 py-2"
          style={{
        ...
        >
          {NAV_ITEMS.map(({ href, label, Icon }) => {
            const active = href === '/'
              ? pathname === '/' || (pathname.startsWith('/trips') && pathname !== '/trips/new')
        ...
        <Link
          ...
          className={cn(
            'flex h-[58px] w-[58px] items-center justify-center rounded-full border transition-all duration-200 active:scale-[0.96]',
            'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400',
            newTripActive
              ? 'border-blue-300/55 bg-ink-200/90 text-blue-100 ring-1 ring-blue-300/20'
              : 'border-white/[0.05] bg-ink-100/82 text-fog-500 hover:bg-ink-200/90 hover:text-blue-200',
          )}
          style={{
            backdropFilter: 'blur(28px) saturate(190%)',
            WebkitBackdropFilter: 'blur(28px) saturate(190%)',
  ...
  return (
    <span
      className={cn(
        ...
        active ? 'bg-blue-400/16 text-blue-100' : 'bg-ink-300/80 text-fog-500',
      )}
      aria-hidden="true"
    >
      <Luggage size={20} strokeWidth={2.25} />
      <span
        className={cn(
          'absolute -right-0.5 -top-0.5 flex h-4 w-4 items-center justify-center rounded-full text-[13px] font-semibold leading-none',
